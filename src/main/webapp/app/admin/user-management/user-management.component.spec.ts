...
describe('UserManagement Component', () => {
  ...
  describe('setActive', () => {
    it('Should update user and call load all', async () => {
      ...
      // WHEN
      userManagement.setActive({ id: 'ABC' }, true);
      await userManagement.$nextTick();

      // THEN
      expect(axiosStub.put.calledWith(`api/admin/users`, { id: 'ABC', activated: true })).toBeTruthy();
      expect(axiosStub.get.calledWith(`api/admin/users?sort=id,asc&page=0&size=20`)).toBeTruthy();
    });
  });
  ...
  describe('confirmDelete', () => {
    it('Should call delete service on confirmDelete', async () => {
      ...
      // WHEN
      userManagement.prepareRemove({ login: 'ABC' });
      userManagement.deleteUser();
      await userManagement.$nextTick();

      // THEN
      expect(axiosStub.delete.calledWith('api/admin/users/' + 'ABC')).toBeTruthy();
      expect(axiosStub.get.calledWith(`api/admin/users?sort=id,asc&page=0&size=20`)).toBeTruthy();
    });
  });
