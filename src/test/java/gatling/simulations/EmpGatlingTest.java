...
/**
 * Performance test for the Emp entity.
 */
public class EmpGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all emps").get("/api/emps").headers(headers_http_authenticated).check(status().is(200)))
                ...
                .exec(
                    http("Create new emp")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_emp_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created emp").get("${new_emp_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created emp").delete("${new_emp_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Emp entity").exec(scn);
