...
/**
 * Performance test for the Dept entity.
 */
public class DeptGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all depts").get("/api/depts").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new dept")
                        .post("/api/depts")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"deptName\": \"SAMPLE_TEXT\"" + ", \"deptAddress\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_dept_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created dept").get("${new_dept_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created dept").delete("${new_dept_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Dept entity").exec(scn);
